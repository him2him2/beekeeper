...
func CheckChunkFound(c bee.Cluster, o Options, pusher *push.Pusher, pushMetrics bool) (err error) {
	...
	if err := c.Nodes[pivot].UploadChunk(ctx, chunk); err != nil {
	...
	ok, err := c.Nodes[pivot].PinChunk(ctx, chunk.Address())
	if err != nil {
		return fmt.Errorf("node %d: %w", pivot, err)
	}
	if !ok {
	...
	fmt.Printf("node %d: pinned uploaded chunk\n", pivot)
	...
	has, err := nodeHasChunk(c.Nodes[pivot], chunk.Address())
	if err != nil {
